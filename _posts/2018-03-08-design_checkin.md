...
### Mobile App
This design is a mobile app that focuses on the user’s experience while they are going about their daily life. First, when a user finds something they want to share, they can pin the location and attach a video, photo, sound bite, or creation. Next, this design employs a competitive mode to motivate exploration, where users can compete against each other in different difficulty modes to earn points. Third, users can ask a question by either placing a pin for location specific questions, or they can specify a general question without a pin, which would appear in a separate Q&A feed. Finally, users can get answers for questions when they tag topics, which will take them to different verified websites, or when other users post comments in response to their question.
...
### Smartwatch
This design is an application for a smartwatch. It will take advantage of the location services that a smartwatch offers as well as the ability of the watch to pair with a smartphone which can play audio to the user. A user will be able to share educational information by leaving voice messages and audio at locations. Users can also get questions answered when walking around because the app can prompt the user to answer a multiple choice question about a location that another user has posted, which will then be shared with others when they walk around this location. Additionally, users can find motivation to go outdoors by utilizing step counting to remind the user to meet daily step targets. Finally, users can share personal creations by recording speech or singing and then pinning it to certain locations. When other users visit that location, they can listen to their creations.
...
### Desktop
This design is a desktop application meant for detailed reviewing of Pins and exploring Pins from the comforts of home. The user will be able to hold anonymous discussions by viewing Pins, which are associated with a thread of comments that the user can reply to. The identity of each user can be revealed depending on each user’s discretion. Users can also find motivation to go outdoors by keeping track of which Pins the user looks at and when, and then suggesting Pins that the user would want to view in person. Additionally, the user can provide educational information because they will have access to more resources than when they are out and about. Finally, the user can share creations that were made with the resources of home because they will have the freedom to take their time perfecting their work.
